 ...
 *     use get_string to provide line-edit capabilities. Shortened the code
 *     by pages.
 */
#include "defs.h"
#include "ext.h"
...
void old_getpost(khflag, fp, tmpstr)
int *khflag;
FILE *fp;
char *tmpstr;
{
...
char    thisline[81];		/* array to save current line		*/
        ...
        if (*thisline) 
	{
        ...
        if (fputs( thisline, fp ) < 0) tempfileerror();
        if (putc( '\n', fp ) < 0) tempfileerror();
        *khflag = 1;
      }

      if (fflush(fp) < 0) tempfileerror();
      if (prompt(fp, khflag, '\n') < 0)
      {
	free (tmpstr);
	colour_so_far ("", -1);
...
void new_getpost(khflag, fp, tmpstr)
int *khflag;
FILE *fp;
char *tmpstr;
{
char *textbuf;
    ...
    for(;;)
    {
      ...
      *khflag = 0;
      ...
      if (fputs( textbuf, fp ) < 0) tempfileerror();
      *khflag = 1;
...
char   *tmpstr;	/* to store the colour coming from the previous line  -Flint */
  ...
  if((upload == 1) || (upload == 0)) 	/* line editing via get_string */
  {
...
int     size;
int     lines;
int	last_was_ctrl_a = 0; /* Flint: the name says it all =); used for 'p' */
char    thisline[80];
char lastchar;
  ...
  for (i = 0;;)
  {
	...
	/*
	 * The DOC'ish way was to send a <c>ontinue-character-"command" from
	 * right here. I have no idea why they wanted to make it like that,
	 * but i do know I won't have it like that. I was quite confused for
	 * some time... (c:
	 */
	...
	continue;
      ...
      case 'C':
	printf("Continue...\r\n");
	colorize(colour_so_far ("", 0));	/* set the previous colour */
	break;
...
/*	  cprintf("Print formatted\r\n\n%s\1a\1c", saveheader); */
	...
	{
	  i++;
	  if (chr == COLORCHAR) /* Flint: added this to set last_was_ctrl_a */
	  {
	    last_was_ctrl_a = 1;
	    continue;
	  }
	  ...
	  else
	  {
	    if (chr == '\n')
	      putchar('\r');
	    if (last_was_ctrl_a) /* Flint: colour code? Then colourize! =) */
	    {
	      ColourChar(chr);
	      last_was_ctrl_a = 0;
	    }
...
/*
 * KHCHANGE - nothing to receive these at the BBS-end.
 *
 *     case 'Q':
 *     case 't':
 *     case 'T':
