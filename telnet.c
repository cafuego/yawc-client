 ...
 * to "fake it" with the BBS) with some extensions to allow the extra
 * features the client provides.  The telnet stuff is unimportant, but it'll
 * confuse the BBS if you alter it.
 * 
 ...
 * connects, after that the BBS acts differently, letting the client know
 * when something in particular is being done (entering an X message,
 * posting, etc.) to allow the client to handle that however it wants, as
 * well as providing extra protocol for the start/end of messages/X's and the
 * special who list.
 * 
 ...
 * passed to it (they count them in the same manner) and throws away the
 * excess on its side.  The client has buffered this excess on its side and
 * therefore make it available locally instead of having it lost forever.
 * Boy, that was a pisser when I realized I'd have to do that!
 * 
 * 
    ...
    int how;
{
	...
	if (!how && buf[i] == 1)
	    i++;
	else if ((buf[i] >= 32 || buf[i] == TAB || buf[i] == 1
		  || buf[i] == '\n' || buf[i] == '\r') && (buf[i] < DEL))
	    tmpbuf[j++] = buf[i];
    tmpbuf[j] = 0;
    strcpy(buf, tmpbuf);
    ...
    register int c;
{
    ...
    /* here's a KHaglund'ish hack: a ^D (Ctrl-D) is sent right before the
     * name so that we know where it is. (the name that is)
     * 
     * Ew! No! We do things the Monolith-Thanatos-Jassyca way, now. Flint, MLPT
     * and I reckoned this was a much better way of marking names out.
     * 
     * Trouble is, it's a little confusing to do in the wholist, so Thanatos
     * doesn't. */
	...
	/* If we can't find the correct tags, we try doing it the old YAWC
	 * way. */
	sp = (char *)mystrchr(wholistbuf, '\4');
	if (sp == NULL) {
	    /* Still didn't work. So we try the worst kludge method yet,
	     * which is ridiculous. */
	...
	if (sp != NULL) {
	    ...
	    /* we have now got a 20 character-string with spaces after the
	     * real name, so we'll remove 'em: */
    ...
    register int c;
{
	...
	if (whop == 1) {
		...
		for (; col++ < savewhop;) {
		    ...
		    /* "%c%c%-19s%d:%02d - %s\r\n", junk & 0x80 ? '*' : '.',
		     * *junk & 0x7f, junk + 1, *savewho[col - 1] / 60,
		     * *savewho[col - 1] % 60, saveinfo[col-1]); */
			    "%c%c%-19s%d:%02d\n",
			    *junk & 0x80 ? '*' : '.', *junk & 0x7f,
	    ...
	    *junk &= 0x7f;
		...
		--*who;
		if (col <= 60) {
		    /* sprintf(work, (userflags & US_ANSICOLORS) ?
		     * "@Y%c%-20s@R%d:%02d@G - @C%s\r\n" : "%c%-20s%d:%02d -
		     * %s\r\n", who[1] & 0x80 ? '*' : '.', junk, who / 60,
		     * *who % 60, info[c]); */
    ...
    register int c;
{
    switch (c) {
			...
			/* This is sent/received when the BBS thinks the
			 * client is 'inactive' as per the normal 10 minute
			 * limit in the BBS -- the actual inactive time limit
			 * for a client is an hour, this is done to make sure
			 * the client is still alive on this end so dead
			 * connections can be timed out and not be ghosted
			 * for an hour. */
			...
			if (idletime < 6 || DOC) {
			    netput(IAC);
			    netput(CLIENT);
			    /* Old style. Not sure if it's even sent,
			     * actually. */
			}
			break;
			...
			/* This code is used by the bbs to signal the client
			 * to synchronize its count of the current byte we
			 * are on.  We then send back a START to the bbs so
			 * it can synchronize with us.  NOTE:  If it becomes
			 * necessary in the future to introduce incompatible
			 * changes in the BBS and the client, this will be
			 * how the BBS can differentiate between new clients
			 * that understand the new stuff, and old clients
			 * which the BBS will probably still want to be
			 * compatible with. Instead of START a new code can
			 * be sent to let the BBS know we understand the new
			 * stuff and will operate accordingly.  If there are
			 * multiple different versions of the BBS to worry
			 * about as well (gag!) then more logic would be
			 * needed, I refuse to worry about this case, if it
			 * comes about it'll be after I ever have to worry
			 * about or maintain that BBS code! */
			...
			/* telnet DO/DONT/WILL/WONT option negotiation
			 * commands (ignored) */
		    case DO:
			state = TS_DO;
			break;
...
handle_lines(c)
    register int c;
{
    /* we are receiving an X message header; check if an enemy is the author,
     * if so, kill the X. */
	...
	if (xmsgbufp < 10) {
	    ...
	    /* it was (sp + 6) below here earlier, but the colorcodes should
	     * make it (sp + 8) if I'm not mistaken. */
     ...
     * 
     * DWD Comment: Trust me. It will. :~) I've not handled it yet, but it's a
     * matter of time.
     * 
	...
	if (postbufp < 50) {
	    ...
	    postbuf[postbufp] = 0;
	    strcpy(junk, postbuf);
	    /* strcpy(saveheader, postbuf); strcat(saveheader, "\r\n"); */
	...
	if (!DOC)
	    process_YAWC_wholist(c);	/* the DOC routine works
					 * character-wise */
    ...
    register int c;
{
	...
	/* If we are more out of sync than our buffer size, we can't recover.
	 * If we are out of sync but not so far out of sync we haven't
	 * overrun our buffers, we just go back into our buffer and find out
	 * what we erroneously sent over the network, and reuse it.
	 * 
	...
	switch (*buf) {
			...
			case G_STRWR:	/* get string, wordwrapped	 */
			    ...
			    /* this here means you cannot get a string longer
			     * than 255 chars, since buf[1] is a character... */
    ...
    char c;
{
    ...
    if ((escape_seq_state == 1) && ((c == '7') || (c == '8'))) {	/* color storing /
									 * restoring */
	switch (c) {
			case '7':
    ...
    register int c;
{
    ...
    switch (state) {
			...
			/* if we're receiving (updating) the userflags from
			 * the BBS. */

			else if (updateflag) {

			    /* after IAC UPDATE, these flags are sent: 1
			     * US_ANSICOLORS, 2 US_NOFLASH, 3 US_NOBOLDCOLORS */
			    ...
			    switch (updateflag) {
						...
						if (c == 0)
						    ...
						    userflags |= US_NOBOLDCOLORS;
						updateflag = 0;	/* this was the last
								 * flag sent	 */
						break;
			...
			/* now here first comes a check if we got a
			 * newline-character, which means we've got a line of
			 * something. this 'something' then has to be handled
			 * appropriately. */
			...
			/* handle various telnet and client-specific IAC
			 * commands */
			...
			/* Ignore next byte (used for ignoring negotations we
			 * don't care about) */
		    case TS_VOID:
		    ...
		    case TS_DO:
			switch (c) {
					case TELOPT_BBSPOST:	/* Yeah, I can handle
								 * them, because I'm a
								 * big CLient. */
					case TELOPT_TIMING_MARK:	/* So that BBS's can
									 * ping CLients. */
					case TELOPT_NAWS:
					case CLIENT_OPTIONS:
			...
			switch (c) {
					...
					case TELOPT_TERMINAL_TYPE:
					    state = TS_IGNORE_SB;
					    current_sb = c;	/* keep track on which
								 * SB we're in */
					    break;
					default:
					    state = TS_IGNORE_SB;	/* This should never be
									 * reached though. */
					    break;
			}
			break;

		    case TS_IGNORE_SB:	/* wait for IAC SB, ignore the
					 * contents */
			if (c == IAC)
		    ...
		    case TS_IGNORE_SB_WAS_IAC:	/* wait for IAC SB, ignore
						 * the contents */
			state = TS_DATA;
			if (c == SE) {
			    sp = (char *)malloc(80);
			    ...
			    state = TS_IGNORE_SB;	/* not an IAC SE - back
							 * into ignore mode */
 ...
 * 
 * KH says: Hmm, currently this isn't used or necessary at all, but it could be
 * a good thing really... *nod* I'll make something to receive the data at
 * the BBS-side. and remove the usersupp->screenlength... ? */
void
sendnaws()
{
    char s[10];
 ...
 * negotations, we just tell the bbs what it needs to hear, since we don't
 * need to negotiate because we know the correct state to put the terminal
 * in. The BBS (the queue daemon actually) is kludged on its end as well by
 * the IAC CLIENT command. */
void
telinit()
{
    char s[39];
	...
	netput(IAC);
	netput(CLIENT);
	/* this sendnaws() should not be here, since the yawcserver is just
	 * checking for the CLIENT-signal above here, and skips the rest. it
	 * could perhaps be in the START-case? */
	sendnaws();
	/* And more to the point, we don't know if the BBS supports NAWS yet,
	 * either. What a shite protocol. Makes me ashamed to develop it. */
    }
