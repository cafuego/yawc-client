...
/* Wait for the next activity from either the user or the network -- we
 * ignore the user while we have a child process running.  Returns 1 for user
 * input pending, 2 for network input pending, 3 for both. */
int
waitnextevent()
{
    fd_set fdr;
    ...
    for (;;) {
	for (i = 0; i < MAXSYS; i++) {	/* scanning for movements in other
					 * sessions */
...
/* Locate the bbsrc file.  Usually is ~/.yawcbbsrc, can be overriden by the
 * BBSRC environment variable if it is set. Returns a pointer to the file via
 * openbbsrc(). */
FILE *
findbbsrc()
{
...
int
real_connect(netFd, sad, szsa, how)
    int netFd;
    struct sockaddr *sad;
    int szsa;
    int how;
...
/* Open a socket connection to the bbs.  Defaults to BBSHOST with port
 * BBSPORT but can be overridden in the bbsrc file if/when the source to the
 * ISCA BBS is released and others start their own on different machines
 * and/or ports. */
void
connectbbs(how)
    int how;
...
/* Suspend the client.  Restores terminal to previous state before
 * suspending, puts it back in proper mode when client restarts, and checks
 * if the window size was changed while we were away. */
void
suspend()
{
    resetterm();
...
/* Quits gracefully when we are given a HUP or STOP signal. sig_return_t */
void
bye()
{
    myexit();
...
/* Handles the death of the child by doing a longjmp back to the function
 * that forked it.  We get spurious signals when the child is stopped, and to
 * avoid confusion we don't allow the child to be stopped -- therefore we
 * attempt to send a continue signal to the child here.  If it fails, we
 * assume the child did in fact die, and longjmp back to the function that
 * forked it.  If it doesn't fail, the child is restarted and the user is
 * forced to exit the child cleanly to get back into the main client. */
sig_return_t
reapchild()
{
    wait(0);
 ...
 * in general the same thing will happen on one end or the other, so you
 * won't speed things up at all by changing this, the sleep is there for your
...
/* Run the command 'cmd' with argument 'arg'.  Used only for running the
 * editor right now.  In order to work properly with all the versions of Unix
 * I've tried to port this to so far without be overly complicated, I have to
 * use a setjmp to save the local stack context in this function, then
 * longjmp back here once I receive a signal from the child that it has
 * terminated. So I guess there actually IS a use for setjmp/longjmp after
 * all! :-) */
void
run(cmd, arg)
    char *cmd;
    ...
    char *version;
{
     ...
     * third arguments to setvbuf() reversed.  Luckily for me, however, on
     * almost every system NULL == _IOFBF == 0!  If this causes problems
     * (i.e. fflush() causes a segmentation fault) an appropriate #ifdef will
     * be needed for the system in question. */
